...
async function loadLevelData() {
  try {
    const response = await fetch('nivel_1_completo.json');
...
function loadProgress() {
  const saved = localStorage.getItem('nivel1_progress');
  if (saved) {
...
function saveProgress() {
  localStorage.setItem('nivel1_progress', JSON.stringify({
  ...
  if (solvedExercises.size === allExercises.length && allExercises.length > 0) {
    localStorage.setItem('nivel1_completed', 'true');
    localStorage.setItem('nivel1_completed_date', new Date().toISOString());
  }
}
...
function initUI() {
  ...
  document.getElementById('levelBadge').textContent = 'NIVEL 1';
}
...
function loadExercise(index) {
  ...
  const savedJust = localStorage.getItem(`nivel1_justification_${ex.id}`);
...
function saveJustification() {
  ...
  localStorage.setItem(`nivel1_justification_${ex.id}`, userText);
