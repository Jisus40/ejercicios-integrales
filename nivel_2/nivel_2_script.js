...
async function loadLevelData() {
  try {
    const response = await fetch('nivel_2_completo.json');
...
function loadProgress() {
  const saved = localStorage.getItem('nivel2_progress');
  if (saved) {
...
function saveProgress() {
  localStorage.setItem('nivel2_progress', JSON.stringify({
  ...
  if (solvedExercises.size === allExercises.length && allExercises.length > 0) {
    localStorage.setItem('nivel2_completed', 'true');
    localStorage.setItem('nivel2_completed_date', new Date().toISOString());
  }
}
...
function initUI() {
  ...
  document.getElementById('levelBadge').textContent = 'NIVEL 2';
}
...
function loadExercise(index) {
  ...
  const savedJust = localStorage.getItem(`nivel2_justification_${ex.id}`);
...
function saveJustification() {
  ...
  localStorage.setItem(`nivel2_justification_${ex.id}`, userText);
